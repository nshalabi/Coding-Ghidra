 ...
 */
public abstract class LibHeadlessScript extends GhidraScript {

	/**
	 * Options for controlling disposition of program after the current script completes.
	 */
	public enum LibHeadlessContinuationOption {
		/**
	...
	private LibHeadlessAnalyzer headless = null;

	private LibHeadlessContinuationOption currentOption = LibHeadlessContinuationOption.CONTINUE;
	private LibHeadlessContinuationOption scriptSetOption = null;
	 ...
	 */
	void setHeadlessInstance(LibHeadlessAnalyzer ha) {
		headless = ha;
	}
	 ...
	 */
	void setInitialContinuationOption(LibHeadlessContinuationOption option) {
	 ...
	 */
	LibHeadlessContinuationOption getContinuationOption() {
	 ...
	 */
	public void setHeadlessContinuationOption(LibHeadlessContinuationOption option) {
	 ...
	 * @see #setHeadlessContinuationOption(LibHeadlessContinuationOption)
	 */
	public LibHeadlessContinuationOption getHeadlessContinuationOption() {
		if (scriptSetOption == null) {
			return LibHeadlessContinuationOption.CONTINUE;
		}
	...
	public void runScript(String scriptName, String[] scriptArguments, GhidraState scriptState)
		...
		if (scriptSource != null) {
			...
			isHeadlessScript = script instanceof LibHeadlessScript ? true : false;
			...
			if (isHeadlessScript) {
				((LibHeadlessScript) script).setHeadlessInstance(headless);
				((LibHeadlessScript) script).setRunningInnerScript(true);
			}
			...
			if (isHeadlessScript) {
				LibHeadlessContinuationOption innerScriptOpt =
					((LibHeadlessScript) script).getHeadlessContinuationOption();

				if (innerScriptOpt != null) {
					resolveContinuationOptionWith(innerScriptOpt);
				}

				((LibHeadlessScript) script).setRunningInnerScript(false);
			}
	 ...
	 */
	private void resolveContinuationOptionWith(LibHeadlessContinuationOption opt) {
			...
			case CONTINUE_THEN_DELETE:
				switch (opt) {
					...
					case ABORT_AND_DELETE:
						currentOption = LibHeadlessContinuationOption.ABORT_AND_DELETE;
						break;
