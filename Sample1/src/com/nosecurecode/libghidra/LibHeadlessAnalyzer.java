...
import com.nosecurecode.libghidra.LibHeadlessScript.LibHeadlessContinuationOption;
 ...
 * broken out into their own class: {@link LibHeadlessOptions}.  This class is intended to be used
 * one of two ways:
 * <ul>
 *   <li>Used by {@link LibGhidra} to perform headless analysis based on arguments specified
...
public class LibHeadlessAnalyzer {
	...
	private static LibHeadlessAnalyzer instance;
	 ...
	 */
	public static LibHeadlessAnalyzer getLoggableInstance(File logFile, File scriptLogFile,
														  boolean useLog4j, LibProgramHandler handler) throws IllegalStateException, IOException {
		...
		instance = new LibHeadlessAnalyzer();

		// Set our program handler
		instance.programHandler = handler;
	 ...
	 */
	private LibHeadlessContinuationOption runScriptsList(List<Pair<String, String[]>> scriptsList,
			Map<String, ResourceFile> scriptFileMap, GhidraState scriptState,
			LibHeadlessContinuationOption continueOption) {

		ResourceFile currScriptFile;
		LibHeadlessContinuationOption retOption = continueOption;
			...
			for (Pair<String, String[]> scriptPair : scriptsList) {
				...
				if (isHeadlessScript) {
					if (scriptSuccess) {
						...
						if ((retOption == LibHeadlessContinuationOption.ABORT) ||
							(retOption == LibHeadlessContinuationOption.ABORT_AND_DELETE)) {
							return retOption;
						}
						...
						return LibHeadlessContinuationOption.ABORT;
					}
				}
			}
		}
	...
	private boolean analyzeProgram(String fileAbsolutePath, Program program) {
		...
		LibHeadlessContinuationOption scriptStatus = LibHeadlessContinuationOption.CONTINUE;
			...
			if (options.analyze) {
				...
				if (options.perFileTimeout == -1) {
					...
					LibHeadlessTimedTaskMonitor timerMonitor =
						new LibHeadlessTimedTaskMonitor(options.perFileTimeout);
